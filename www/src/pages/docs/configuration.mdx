...
kprobe uses three Kafka topics in the current local pipeline, plus one planned OpenTelemetry topic:

| Topic             | Content                             | Produced by | Consumed by   |
| ----------------- | ----------------------------------- | ----------- | ------------- |
| `kernel.raw`      | Raw kernel events as JSON           | eBPF probe  | Vector        |
| `kernel.enriched` | Parsed kernel events                | Vector      | Causal engine, API stream |
| `kernel.dlq`      | Invalid kernel event payloads       | Causal engine | Operators |

`otel.spans` is planned for OpenTelemetry span payloads, but it is not produced by the current local Vector config yet.
