...
```bashhelm repo add kprobe https://charts.kprobe.io
helm repo update

Install kprobe into its own namespace:

```bashhelm install kprobe kprobe/kprobe
...
You should see pods for each component:NAME                              READY   STATUS    RESTARTS
kprobe-probe-xxxxx                1/1     Running   0         # DaemonSet — one per node
...
```bashkubectl port-forward svc/kprobe-dashboard 3000:3000 -n monitoring
...
```bashgit clone https://github.com/YHQZ1/kprobe
cd kprobe
...
```bashhelm uninstall kprobe -n monitoring
