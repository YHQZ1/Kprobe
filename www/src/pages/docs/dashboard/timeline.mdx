...
The timeline view shows all kernel events and application events on a single horizontal timeline, across all services and nodes simultaneously. It is the best view for understanding the exact sequence and timing of events.

## Layout

The timeline is organized into horizontal swimlanes, one per service or kernel component:

- Each **service** gets its own swimlane showing its application events (requests received, writes initiated, responses sent)
...
The selected time range can be exported as a CSV of all events for offline analysis. The CSV includes all fields: timestamp (nanoseconds), PID, TID, CPU, event type, duration, and application context.
